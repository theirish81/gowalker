...
type Functions map[string]func(scope any, params ...any) (any, error)
...
func (f *Functions) Add(key string, function func(data any, params ...any) (any, error)) *Functions {
	(*f)[key] = function
	return f
}
...
func (f *Functions) size(scope any, _ ...any) (any, error) {
	if scope == nil {
...
func (f *Functions) split(scope any, params ...any) (any, error) {
	if len(params) < 1 {
		return nil, errors.New("separator not provided")
	}
	if val, ok := scope.(string); ok {
		if sep, ok := params[0].(string); ok {
...
func runFunction(expr string, data interface{}, functions Functions) (bool, interface{}, error) {
	if fx := extractFunctionName(expr); fx != "" {
		params := retypeParams(extractParameters(expr))
